...
class SN:
    '''
    Define skin property of a SN and the following:
    - POS: a dict collecting the locations of all SNs
    '''
    COLOR = (0, 0, 255)           # blue color for SNs
    POS = {0:Pos(4.5, 2.5),       # list of SNs, including their location
           1:Pos(11.5, 6.5),
           2:Pos(10, 3),
           3:Pos(2, 5),           # additional SNs for improved testing
           4:Pos(7, 1),
           5:Pos(13, 4),
           6:Pos(5, 8),
           7:Pos(12, 10)}         # total of 8 SNs
    RATE = {}                     # to be filled with the rate per SN per cell
            ...
            for row in range(Frame.ROWS):
                ...
                for obstacle in OBSTACLE.POS_LIST:
                    ...
                    if line1.intersects(rect1):
                        ...
                        break
    ## consolidate blockage from all SNs into `BLOCKAGE[col][row]`
    for id in SN.POS:
        for col in range(Frame.COLS):
            for row in range(Frame.ROWS):
...
class COMM:
    ...
    def get_sn_rate_matrix():
        sn_rate = {}
        for sn_id,sn_pos in SN.POS.items():
            sn_rate[sn_id] = [[0 for _ in range(Frame.ROWS)] 
                              for _ in range(Frame.COLS)]    # create a 2D array with zeroes
            for col in range(Frame.COLS):
                for row in range(Frame.ROWS):
                    d = Frame.cell_distance(sn_pos,Pos(col,row),ground_to_air=True) \
                        * Frame.INITIAL_METER_PER_PIXEL
                    sn_rate[sn_id][col][row] = COMM.get_rate(d, SHADOW.NLOS[sn_id][col][row])
        return sn_rate

## fill SN.RATE matrix, use this one time calculation 
## to avoid repeating the same calculation over and over again,
SN.RATE = COMM.get_sn_rate_matrix()
...
class UAVEnv(UAVEnvBase):
    ...
    def step(self, action):
        ...
        ## calculate reward
        all_rates = []
        for sn_id in SN.RATE:
            all_rates.append(SN.RATE[sn_id][self.uav_pos.col][self.uav_pos.row])
    ...
    def render(self, fps=60):
        ...
        ## draw SNs
        for pos in SN.POS.values():
            (x,y) = Frame.cell_center_xy(Pos(pos.col,pos.row))
            pygame.draw.circle(self.screen, SN.COLOR, (x,y), Frame.object_size())
