...
class SARSA(RL):
    ...
    def __init__(self, num_actions, exploration=True):
        ...
        ## ML hyperparameter
        self.alpha: float = 0.5    # learning rate (higher than Q-Learning to compensate for on-policy learning)
        self.gamma: float = 0.99   # discount factor (same as Q-Learning for fair comparison)
    ...
    def load_data(self) -> int:
        '''Load Q-table from `{ai.name}-load.json` in uploads directory only.'''
        # Only check uploads directory
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        uploads_dir = os.path.join(project_root, "uploads")
        filename = os.path.join(uploads_dir, f"{self.name}-load.json")
    ...
    def save_data(self, round_id) -> bool:
        ...
        # Get project root directory (one level up from rl/)
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        uploads_dir = os.path.join(project_root, "uploads")
        # Save to uploads directory if it exists, otherwise to project root
        if os.path.exists(uploads_dir):
            filename = os.path.join(uploads_dir, strftime(f"{self.name}-[%Y-%m-%d][%Hh%Mm%Ss].json", localtime()))
        else:
            filename = os.path.join(project_root, strftime(f"{self.name}-[%Y-%m-%d][%Hh%Mm%Ss].json", localtime()))
    ...
    def get_action(self, state, reward=None, done=False) -> int:
        '''
        ...
        ## Only choose action if episode is not done
        if not done:
            ...
            ## Episode is done, no next action
            a1 = None
        ...
        if self.current_state is not None and reward is not None:
            if done:
                ## Episode terminated/truncated: future value is 0
                future_value = 0.0
            else:
                ## Normal case: use Q(s1, a1)
                future_value = self.q(s1)[a1]
            self.q(s)[a] = self.q(s)[a] \
                    + self.alpha * (reward + self.gamma*future_value - self.q(s)[a])

        ## return the action, also save (state,action) pair
        self.current_state = state
        self.current_action = a1
        return a1 if a1 is not None else 0  # return 0 as default if episode is done
