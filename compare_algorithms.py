...
def load_results_from_directory(results_dir="results"):
    ...
    if not os.path.exists(results_path):
        # Check if results directory exists in current working directory as fallback
        cwd_results = os.path.join(os.getcwd(), results_dir)
        if os.path.exists(cwd_results):
            results_path = cwd_results
            print(f"Note: Using results directory from current working directory: {results_path}")
        else:
            raise FileNotFoundError(
                ...
                f"Please run uav.py first to generate results in the 'results' directory."
            )
    ...
    if not json_files:
        raise FileNotFoundError(
            f"No result files found in '{results_path}' directory.\n"
            f"  Found files: {os.listdir(results_path) if os.path.exists(results_path) else 'N/A'}\n"
